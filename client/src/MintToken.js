...
class MintToken extends React.Component {
  ...
  createToken = async()=> {
		...
		const op = await contracts.stripeMarket.methods.createItem(contracts.nft._address, currTokenId, Number(this.state.price)).send({from: this.state.accounts[0], value: listingPrice})
